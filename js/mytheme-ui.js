var MyTheme = {
	...
	'isver':function(){
		if($(".isver").length && $(".isver").attr('data-ver')=="版权"){$(".myui-content__detail p font").html("版权下架");$(".myui-panel.myui-panel-bg.clearfix").eq(0).before('<div class="myui-panel myui-panel-bg clearfix text-center" id="islun">版权投诉，已下架，请官网观看</div>');$('.btn.btn-warm').attr('href','javascript:void(0)').html("版权下架");}
	},
	'Other': {
		'Headroom': function() {
		...
		'Player': function() {
			if(!MyTheme.Browser.useragent.mobile){$("#rpch").addClass("embed-responsive embed-responsive-16by9 clearfix");}
			...
			if($(".player-fixed").length){
				if(!MyTheme.Browser.useragent.mobile){
					$(window).scroll(function(){
						if($(window).scrollTop()>LeftHeight){//window.outerHeight
							//PlayerSide.toggle();
							if(PlayerSide.css("display")==='none') {
								PlayerSide.css("display","block");
								PlayerLeft.css("width","");
								$("#player-sidebar-is").html("<i class='fa fa-angle-right'></i>");
							}
